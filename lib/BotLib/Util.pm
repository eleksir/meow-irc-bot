package BotLib::Util;
# Содержит утилитарные функции общего характера
...
our @EXPORT_OK = qw (timems trim urlencode utf2b64 utf2sha1 utf2md5 utf2sha1hex utf2sha224hex utf2sha256hex
                     ...
                     cleanexpireddata rakedata deletedata  cleardata SigHandler SigIntHandler SigTermHandler
                     SigQuitHandler);
...
# Берёт от даденной строки crc32
# $str utf2crc32hex($str)
sub utf2crc32hex {
	my $string = shift;
...
sub runcmd {
	...
	if ($CHILD_ERROR != 0) {
		...
		# Любой utf-8 из вне надо прогонять через encode
		$ret->{text} = encode ('UTF-8', $out);
	}
...
sub storedata {
	...
	cluck "[ERROR] datadir undefined in config"    unless (defined $c->{datadir});
	cluck "[ERROR] database not defined in config" unless (defined $c->{db}->{$dbname});
...
# Удаляет все ключи со значениями из даденной базы
# undef cleardata($dbname)
sub cleardata {
	my $dbname = shift;
	...
	$cache->clear ();
	$log->debug ("[DEBUG] Db $dbname cleared");
...
sub SigHandler {
	...
	# Мы не должны увидеть это в логах
